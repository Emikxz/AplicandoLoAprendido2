...
export function mostrarDetalles(tarea: Tarea | undefined): void {
  if (!tarea) { console.log("Tarea inválida."); return; }
  ...
  if (tarea.fechaEdicion) console.log("Última edición:", tarea.fechaEdicion);

  const opc = prompt("\n¿Deseas editar esta tarea? (1 = Sí, 0 = No) ").trim();
  if (opc === "1") editarTarea(tarea);
}
